...
type LogEntry = { id: number; kind: "buy" | "sell" | "info"; text: string };
...
export default function LifeSupportSection() {
  ...
  // EKG line — bumps when vit changes
  useEffect(() => {
    const el = ekgRef.current;
    if (!el) return;
    const W = 600;
    const H = 80;
    const tick = 12;
    ...
    while (x < W) {
      const baseline = H / 2;
      // each cycle: flat, small bump, big spike, flat
      const cycle = (x / tick) % 6;
      let y = baseline;
      if (cycle === 1) y = baseline - 6;
      if (cycle === 2) y = baseline + (vit < 30 ? 8 : 4);
      if (cycle === 3) y = baseline - (vit < 30 ? 28 : 22);
  ...
  const status =
    ...
    vit > 15 ? { label: "CRITICAL", color: "bg-hot", pulse: "bg-hot" } :
    { label: "FLATLINE", color: "bg-blood text-bone", pulse: "bg-blood" };

  return (
    <section className="px-5 sm:px-8 py-16 sm:py-24 border-b-[3px] border-ink bg-bone relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto relative grid lg:grid-cols-[1.2fr_1fr] gap-10 lg:gap-14 items-center">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="font-mono font-bold text-[10px] uppercase opacity-60 hidden sm:inline">↓ try the buttons</span>
          </div>

          <blockquote className="font-display uppercase tracking-[-.035em] leading-[.92] text-[36px] sm:text-[56px] lg:text-[76px]">
            Holders don{"'"}t<br />
          ...
          <div className="mt-6 flex items-center gap-3 flex-wrap">
            ...
            <div className="font-mono font-bold text-[10px] uppercase opacity-60">never existed in crypto before</div>
          </div>
        </div>
        ...
        <div className="card !shadow-[8px_8px_0_0_#0a0a0a] bg-bone w-full max-w-md mx-auto lg:mx-0 lg:ml-auto">
          ...
          <div className="bg-ink border-b-[3px] border-ink relative">
            <svg viewBox="0 0 600 80" className="w-full h-[80px] block">
              <polyline
                ref={ekgRef}
                fill="none"
                stroke={vit < 15 ? "#ff4127" : vit < 40 ? "#ffe14a" : "#c6ff3d"}
                strokeWidth="2.5"
                strokeLinecap="square"
                strokeLinejoin="miter"
                style={{ filter: `drop-shadow(0 0 6px ${vit < 15 ? "#ff4127" : vit < 40 ? "#ffe14a" : "#c6ff3d"})` }}
              />
            </svg>
            <div className="absolute top-1.5 left-2 font-mono text-[9px] font-extrabold text-acid/80 uppercase">EKG · 60bpm</div>
            <div className="absolute top-1.5 right-2 font-mono text-[9px] font-extrabold text-acid/80 uppercase">{vit < 15 ? "ARRHYTHMIA" : "SINUS"}</div>
          </div>

          {/* stat row */}
          <div className="grid grid-cols-3 border-b-[3px] border-ink">
            <Stat k="Vitality" v={`${Math.round(vit)}%`} accent={vit < 30 ? "text-blood" : vit < 60 ? "text-ink" : "text-[#1a8c1a]"} />
          ...
          <div className="px-4 py-3.5 border-b-[3px] border-ink">
            ...
            <div className="h-[20px] border-[3px] border-ink bg-bone relative overflow-hidden">
              <div
                ...
                style={{
                  width: `${vit}%`,
                  background: vit < 15 ? "#ff4127" : vit < 40 ? "#ffe14a" : "#c6ff3d",
          ...
          <div className="grid grid-cols-2 border-b-[3px] border-ink">
            <button
              onClick={buy}
              className="font-display text-[18px] uppercase py-4 bg-acid hover:bg-bone transition border-r-[3px] border-ink active:translate-y-[1px]"
            >
              ↑ BUY +12
            </button>
            <button
              onClick={sell}
              className="font-display text-[18px] uppercase py-4 bg-hot hover:bg-bone transition active:translate-y-[1px]"
            >
              ↓ SELL −18
            </button>
          </div>
          ...
          <div className="bg-ink text-bone p-3">
            <div className="font-mono text-[9px] font-extrabold uppercase opacity-60 mb-1.5">agent log</div>
...
function Stat({ k, v, mid, accent }: { k: string; v: string; mid?: boolean; accent?: string }) {
  return (
    <div className={`p-3 ${mid ? "border-x-[3px] border-ink" : ""}`}>
